...
class TestWorkflow(unittest.TestCase):
    ...
    def get_inputs_outputs(self, nodes):
        ...
        actual_outputs = []
        node_tuples = [
            (node.name, node.inputs.items(), node.outputs.items()) for node in nodes
        ]
    ...
    def assert_inputs_set(self, workflow, additional_inputs=None):
        ...
        node_names = [
            name for name in workflow.list_node_names() if name.count(".") == 0
        ]
        for node_name in set(node_names + list(additional_inputs.keys())):
            ...
            other_inputs = (
                additional_inputs[node_name] if node_name in additional_inputs else []
            )
